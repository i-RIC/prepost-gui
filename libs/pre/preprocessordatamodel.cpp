...
void PreProcessorDataModel::importCalcCondition()
{
	...
	QString fname = QFileDialog::getOpenFileName(
		iricMainWindow(), tr("Select file to import"), ProjectLastIODirectory::get(), tr("All importable files(*.cgn *.cgns *.ipro *.yml);;iRIC project file (*.ipro);;CGNS file (*.cgn *.cgns);;YAML file (*.yml)"), &selectedFilter);
	if (fname == "") {return;}
...
void PreProcessorDataModel::exportCalcCondition()
{
	QString fname = QFileDialog::getSaveFileName(
		projectData()->mainWindow(), tr("Select File to Export"), ProjectLastIODirectory::get(), tr("CGNS file (*.cgns);;YAML file (*.yml)"));
	if (fname == "") {return;}
