...
#include "preprocessorgriddataitem_selectediedgescontroller.h"
...
PreProcessorGridDataItem::SelectedIEdgesController::SelectedIEdgesController(PreProcessorGridDataItem* item) :
...
PreProcessorGridDataItem::SelectedIEdgesController::~SelectedIEdgesController()
{
	m_item->renderer()->RemoveActor(m_cellsLinesActor);
}

vtkPointSet* PreProcessorGridDataItem::SelectedIEdgesController::data() const
{
	auto grid = dynamic_cast<v4Structured2dGrid*> (m_item->impl->m_grid->grid());
	if (grid == nullptr) {return nullptr;}

	return grid->vtkIEdgeData()->data();
}

void PreProcessorGridDataItem::SelectedIEdgesController::updateSelectedCells(MouseBoundingBox* box, bool xOr, VTKGraphicsView* view)
{
	...
	if (iRIC::isNear(box->startPoint(), box->endPoint())) {
		...
		auto iEdgeGrid = sGrid->vtkIEdgeData();

		auto view2 = dynamic_cast<VTK2DGraphicsView*> (view);
		vtkIdType cellId = iEdgeGrid->findCell(point[0], point[1], 0, view2->stdRadius(iRIC::nearRadius()), weights);
		if (cellId >= 0) {
		...
		selectedDataIdsVector = m_item->getIEdgesFromVertices(selectedVertices);
	}
