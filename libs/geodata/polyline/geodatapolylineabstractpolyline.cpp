...
void GeoDataPolyLineAbstractPolyLine::updateActorSetting()
{
	...
	// mapping
	if (ds.mapping == GeoDataPolyLine::DisplaySetting::Mapping::Arbitrary) {
		...
		auto colormap = impl->m_parent->geoDataDataItem()->colorMapSettingContainer();

		vtkMapper* mapper = nullptr;

		mapper = colormap->buildCellDataMapper(impl->m_polylineController.polyData(), true);
		impl->m_linesActor->SetMapper(mapper);
		mapper->Delete();

		mapper = colormap->buildCellDataMapper(impl->m_polylineController.pointsPolyData(), true);
		impl->m_pointsActor->SetMapper(mapper);
		mapper->Delete();
	}
}
