...
{
	polygonGroup()->keyPressEvent(event, dynamic_cast<PreProcessorGraphicsViewInterface*>(v));
}

void GeoDataPointmap::PolygonsManager::handleKeyReleaseEvent(QKeyEvent* event, VTK2DGraphicsView* v)
{
	polygonGroup()->keyReleaseEvent(event, dynamic_cast<PreProcessorGraphicsViewInterface*>(v));
}

void GeoDataPointmap::PolygonsManager::handleMouseDoubleClickEvent(QMouseEvent* event, VTK2DGraphicsView* v)
{
	polygonGroup()->mouseDoubleClickEvent(event, dynamic_cast<PreProcessorGraphicsViewInterface*>(v));
}
