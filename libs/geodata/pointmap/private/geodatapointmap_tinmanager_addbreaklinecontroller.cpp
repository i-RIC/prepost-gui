...
void GeoDataPointmap::TINManager::AddBreakLineController::handleMouseMoveEvent(QMouseEvent* event, VTK2DGraphicsView* v)
{
	...
	vtkIdType newPoint = m_manager->m_pointsManager->closestPointWithinRadius(worldPos, radius);

	if (newPoint != m_currentVertex) {
		m_manager->m_parent->pushRenderCommand(new AddPointCommand(false, m_currentVertex != NOT_SELECTED, newPoint, this));
...
void GeoDataPointmap::TINManager::AddBreakLineController::handleMousePressEvent(QMouseEvent* event, VTK2DGraphicsView* v)
{
	...
	m_manager->m_parent->pushModifyCommand(new AddPointCommand(true, true, m_currentVertex, this));
}

void GeoDataPointmap::TINManager::AddBreakLineController::finish()
{
	m_currentVertex = NOT_SELECTED;
	auto v = m_manager->m_parent->dataModel()->graphicsView();
	v->setCursor(Qt::ArrowCursor);
	...
	if (m_breakLine->vertexIndices().size() < 2) {
		...
		m_breakLine->removeActorsFromRenderer(m_manager->m_parent->renderer());
		m_manager->m_breakLines.pop_back();
		delete m_breakLine;
	}
