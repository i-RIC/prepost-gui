...
public:
	GeoDataPointmapCellMapperSetting() : GeoDataMapperSetting() {}
...
public:
	GeoDataPointmapCellMapperT(GeoDataCreator* parent)
		...
		GeoDataCellMapperT<V, DA>::m_caption = "Pointmap cell mapper";
	}
	GeoDataMapperSetting* initialize(bool* boolMap) override {
		...
		GeoDataPointMapT<V, DA>* pointmap = dynamic_cast<GeoDataPointMapT<V, DA>* >(GeoDataCellMapperT<V, DA>::m_geodata);
		if (pointmap->needRemeshing()) {
			pointmap->remeshTINS(true);
		}
		vtkPolyData* tmpgrid = pointmap->delaunayedPolyData();
		double* weights = new double[3];
		if (GeoDataCellMapperT<V, DA>::container()->condition()->isOption()) {
			...
			for (unsigned int i = 0; i < count; ++i) {
				...
				for (int j = 0; j < cell->GetNumberOfPoints(); ++j) {
					GeoDataMapper::m_grid->vtkGrid()->GetPoint(cell->GetPointId(j), point);
					for (int k = 0; k < 3; ++k) {
				...
				if (cellid >= 0) {
					...
					*(boolMap + i) = true;
					...
					GeoDataCellMapperT<V, DA>::m_grid->vtkGrid()->GetPointCells(i, cells);
					double bounds[6];
					for (vtkIdType j = 0; j < cells->GetNumberOfIds(); ++j) {
						vtkIdType cellid = cells->GetId(j);
						double tmpbounds[6];
						GeoDataCellMapperT<V, DA>::m_grid->vtkGrid()->GetCellBounds(cellid, tmpbounds);
						for (int k = 0; k < 3; ++k) {
