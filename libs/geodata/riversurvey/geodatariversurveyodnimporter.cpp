...
bool GeoDataRiverSurveyOdnImporter::importData(GeoData* data, int /*index*/, QWidget* w)
{
	...
	if (! f.open(QIODevice::ReadOnly | QIODevice::Text)) {
		QMessageBox::critical(w, GeoDataRiverSurveyOdnImporter::tr("Error"), GeoDataRiverSurveyOdnImporter::tr("Error occured while opening %1").arg(QDir::toNativeSeparators(setting()->fileName())));
		return false;
	}
