...
GeoDataRiverSurveyCrosssectionWindowDelegate::GeoDataRiverSurveyCrosssectionWindowDelegate()
{
	m_editor = nullptr;
...
void GeoDataRiverSurveyCrosssectionWindowDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
...
QWidget* GeoDataRiverSurveyCrosssectionWindowDelegate::createEditor(QWidget* parent, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
...
void GeoDataRiverSurveyCrosssectionWindowDelegate::setEditorData(QWidget* editor, const QModelIndex& index) const
{
	if (m_crosssection == nullptr) {return;}
	QVariant dat = index.model()->data(index, Qt::DisplayRole);
	if (dat.type() == QVariant::Double) {
...
void GeoDataRiverSurveyCrosssectionWindowDelegate::setModelData(QWidget* editor, QAbstractItemModel* model, const QModelIndex& index) const
{
	QVariant dat = index.model()->data(index, Qt::DisplayRole);
	if (dat.type() == QVariant::Double) {
		// Float. Edit with RealEdit
...
void GeoDataRiverSurveyCrosssectionWindowDelegate::updateEditorGeometry(QWidget* editor, const QStyleOptionViewItem& option, const QModelIndex& /*index*/) const
{
	editor->setGeometry(option.rect);
}

void GeoDataRiverSurveyCrosssectionWindowDelegate::handleCheckboxToggle(bool toggled)
{
	m_model->setData(m_index, toggled);
}

void GeoDataRiverSurveyCrosssectionWindowDelegate::handleEditorDestroy(QObject* e)
{
	if (m_editor == e) {
		m_editor = nullptr;
	}
