...
void RawDataRiverPathPoint::remove(){
	...
	if (m_nextPoint == 0){m_previousPoint->setGridSkip(false);}
	if (m_nextPoint != 0){
...
QVector2D RawDataRiverPathPoint::CtrlPointPosition2D(CtrlPointPosition pos, double d)
{
	...
	case pposCenterToRight:
		...
		break;
	case pposCenterLine:
		return myCtrlPointPosition2D(&RawDataRiverPathPoint::riverCenter, d);
		break;
	case pposLeftBank:
		return myCtrlPointPosition2D(&RawDataRiverPathPoint::leftBank, d);
		break;
	case pposRightBank:
		return myCtrlPointPosition2D(&RawDataRiverPathPoint::rightBank, d);
		break;
	default:
		return QVector2D(0, 0);
	}
...
void RawDataRiverPathPoint::SelectCtrlPointsRegion(
...
void RawDataRiverPathPoint::removeExtentionPointLeft()
{
	crosssection().unsetFixedPointL();
}

void RawDataRiverPathPoint::removeExtentionPointRight()
{
	crosssection().unsetFixedPointR();
}

void RawDataRiverPathPoint::shiftCenter(double shiftWidth)
{
...
QVector2D RawDataRiverPathPoint::crosssectionPosition(double x)
{
...
double RawDataRiverPathPoint::myHeight(RawDataRiverPathPoint::Bank bank, double t0, double t1, double d)
{
	int i = 0;
	double d0 = 0;
	double d1 = m_CtrlSections[i];
...
void RawDataRiverPathPoint::load(QDataStream& s, const VersionNumber& number)
{
	// name
	s >> m_name;
	// position()
...
void RawDataRiverPathPoint::save(QDataStream& s)
{
	// name
	s << m_name;
	// position
...
void RawDataRiverPathPoint::centerToBanksRegion(QVector2D& mins, QVector2D& maxs)
{
...
void RawDataRiverPathPoint::thisToNextRegion(QVector2D& mins, QVector2D& maxs)
{
	double xmin, xmax, ymin, ymax;
	QVector2D leftbank  = crosssectionPosition(m_crosssection.leftBank(true).position());
	QVector2D rightbank = crosssectionPosition(m_crosssection.rightBank(true).position());
...
void addCtrlPointsToVector(QVector<double>& values, unsigned int index, RawDataRiverPathPoint::CtrlPointsAddMethod method)
{
	double t0, t1;
...
void RawDataRiverPathPoint::setWaterSurfaceElevation(double value)
{
	m_waterSurfaceElevationSpecified = true;
	m_waterSurfaceElevationValue = value;
}

bool RawDataRiverPathPoint::waterSurfaceElevationSpecified() const
{
	return m_waterSurfaceElevationSpecified;
}

double RawDataRiverPathPoint::waterSurfaceElevationValue() const
{
...
void RawDataRiverPathPoint::loadFromiRICLibObject(const iRICLib::RiverPathPoint* p)
{
...
void RawDataRiverPathPoint::saveToiRICLibObject(iRICLib::RiverPathPoint* p)
{
