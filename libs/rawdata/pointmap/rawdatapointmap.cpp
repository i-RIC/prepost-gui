...
class RawDataPointMapDeletePointsCommand : public QUndoCommand
{
public:
...
class RawDataPointMapAddPointSetReferenceCommand : public QUndoCommand
{
public:
...
class RawDataPointMapAddPointCommand : public QUndoCommand
{
...
private:
	...
	RawDataPointmapAddPointDialog* m_iPoints;
...
private:
	RawDataPointmap* m_pmap;
	RawDataPointmapInterpolatePoints* m_iPoints;
	vtkSmartPointer<vtkPoints> newpoints;
	vtkSmartPointer<vtkDoubleArray> newvalues;
	vtkSmartPointer<vtkPoints> oldpoints;
	vtkSmartPointer<vtkDoubleArray> oldvalues;
...
private:
	RawDataPointmapBreakLine* m_breakLine;
	RawDataPointmap* m_pointMap;
};

class RawDataPointmapBreakLineAddPointCommand : public QUndoCommand
{
public:
...
class RawDataPointmapBreakLineFinishDefinitionCommand : public QUndoCommand
{
public:
...
class RawDataPointmapBreakLineCancelDefinitionCommand : public QUndoCommand
{
public:
...
void RawDataPointmap::updateInterpShapeData()
{
...
void RawDataPointmap::updateShapeData()
{
...
class RawDataPMInterpLineAddPointCommand : public QUndoCommand
{
public:
...
class RawDataPMPolygonAddPointCommand : public QUndoCommand
{
public:
...
void RawDataPointmap::saveExternalData(const QString& filename)
{
	iRICLib::PointMap* pm = new iRICLib::PointMap();
...
void RawDataPointmap::updateFilename()
{
	m_filename = m_name;
...
void RawDataPointmap::doLoadFromProjectMainFile(const QDomNode& node){
	...
	m_representation = (RawDataPointmapRepresentationDialog::Representation) node.toElement().attribute("representation", "0").toInt();
...
void RawDataPointmap::buildGridFromPolydata()
{
	m_vtkGrid->Reset();
...
void RawDataPointmap::setupActors()
{
...
bool RawDataPointmap::addToolBarButtons(QToolBar* tb)
{
...
void RawDataPointmap::updateZDepthRangeItemCount(ZDepthRange& range)
{
...
QDialog* RawDataPointmap::propertyDialog(QWidget* parent)
{
	RawDataPointmapRepresentationDialog* dialog = new RawDataPointmapRepresentationDialog(parent);
...
void RawDataPointmap::handlePropertyDialogAccepted(QDialog* propDialog)
{
	RawDataPointmapRepresentationDialog* repDialog = dynamic_cast<RawDataPointmapRepresentationDialog*>(propDialog);
...
void RawDataPointmap::updateRepresentation()
{
	m_actor->VisibilityOff();
	...
	case RawDataPointmapRepresentationDialog::Points:
		...
		break;
	case RawDataPointmapRepresentationDialog::Surface:
		m_vtkMapper->SetInputData(m_vtkDelaunayedPolyData);
		m_actor->GetProperty()->SetRepresentationToSurface();
		actorCollection()->AddItem(m_actor);
		break;
	case RawDataPointmapRepresentationDialog::Wireframe:
		m_vtkMapper->SetInputData(m_vtkDelaunayedPolyData);
...
void RawDataPointmap::addCustomMenuItems(QMenu* menu)
{
	menu->addAction(m_editNameAction);
}

void RawDataPointmap::showDisplaySetting()
{
	dynamic_cast<PreProcessorRawdataDataItemInterface*>(parent())->showPropertyDialog();
}

void RawDataPointmap::setupActions()
{
	m_addPointAction = new QAction(tr("&Add New Point"), this);
	m_addPointAction->setIcon(QIcon(":/libs/rawdata/pointmap/images/iconAddPoint.png"));
	...
	m_interpolatePointAction->setIcon(QIcon(":/libs/rawdata/pointmap/images/iconInterpolatePoint.png"));
	...
	m_selectionModePoint = new QAction(QIcon(":/libs/rawdata/pointmap/images/iconPointSelection.png"), tr("Select point"), this);
	...
	m_selectionModePolygon->setIcon(QIcon(":/libs/rawdata/pointmap/images/iconPolygonSelection.png"));
	...
	m_addBreakLineAction->setIcon(QIcon(":/libs/rawdata/pointmap/images/iconBreakLineAdd.png"));
	...
	m_removeBreakLineAction->setIcon(QIcon(":/libs/rawdata/pointmap/images/iconBreakLineDelete.png"));
...
void RawDataPointmap::mousePressEvent(QMouseEvent* event, PreProcessorGraphicsViewInterface* v)
{
	if (event->button() == Qt::LeftButton){
		...
		case meSMInterpCtrlPoint:
			iRICUndoStack::instance().push(new RawDataPMInterpLineAddPointCommand(true, worldX, worldY, -9999., this));
		...
		case meBreakLineAdd:
			// add point.
			iRICUndoStack::instance().push(new RawDataPointmapBreakLineAddPointCommand(false, m_selectedVertexId2, this));
			break;
...
void RawDataPointmap::mouseMoveEvent(QMouseEvent* event, PreProcessorGraphicsViewInterface* v)
{
	...
	case meNormalWithShift:
		...
		break;
	case meSMPolygon:
		iRICUndoStack::instance().push(new RawDataPMPolygonAddPointCommand(false, QPoint(event->x(), event->y()), this));
		break;
	case meSMInterpPoint:
	case meSMInterpPointPrepare:
	case meSMInterpCtrlPoint:
...
void RawDataPointmap::mouseDoubleClickEvent(QMouseEvent* event, PreProcessorGraphicsViewInterface* v)
{
	doubleclick = true;
	bool xOr = ((event->modifiers() & Qt::ShiftModifier) == Qt::ShiftModifier);
...
void RawDataPointmap::selectionModeBox(bool on)
{
...
void RawDataPointmap::selectionModePolygon(bool on)
{
	this->clearPointsSelection();
...
void RawDataPointmap::updateActionStatus()
{
...
void RawDataPointmap::updateMouseEventMode()
{
	double dx, dy;
	...
	case meBreakLineAddNotPossible:
		...
		iRICUndoStack::instance().push(new RawDataPointmapBreakLineAddPointCommand(true, m_selectedVertexId2, this));
		break;
	case meBreakLineRemove:
	case meBreakLineRemoveNotPossible:
...
void RawDataPointmap::remeshTINS(bool nodialog)
{
	QTime time;
	time.start();
...
void RawDataPointmap::addBreakLine()
{
	if (m_mouseEventMode == meBreakLineAdd || m_mouseEventMode == meBreakLineAddNotPossible){
		...
		clearPointsSelection();
		// add new break line.
		InformationDialog::information(preProcessorWindow(), tr("Information"), tr("Break line can be defined by mouse-clicking on the points between which you want to add break line. Finish defining by double clicking, or pressing return key."), "rawdatapointmapaddbreakline");
		iRICUndoStack::instance().push(new RawDataPointmapBreakLineAddCommand(this));
	}
}
...
void RawDataPointmap::removeBreakLine()
{
	if (m_mouseEventMode == meBreakLineRemoveNotPossible || m_mouseEventMode == meBreakLineRemove){
		...
		clearPointsSelection();
		InformationDialog::information(preProcessorWindow(), tr("Information"), tr("Click on the break line you want to remove."), "rawdatapointmapremovebreakline");
		m_mouseEventMode = meBreakLineRemoveNotPossible;
	}
	updateActionStatus();
	updateMouseCursor(graphicsView());
...
void RawDataPointmap::editPointsDelete()
{
	...
	iRICUndoStack::instance().push(new RawDataPointMapDeletePointsCommand(tr("Delete Points"), selectedVertices(), this));
	this->clearPointsSelection();
}

void RawDataPointmap::editPointsExport()
{
...
void RawDataPointmap::editPointsLessThan()
{
	...
	iRICUndoStack::instance().push(new RawDataPointMapDeletePointsCommand(title.arg(dialog.limitValue()), deletedPoints, this));
	clearPointsSelection();
}
...
const QPolygonF RawDataPointmap::polygon()
{
	QPolygonF ret;
	vtkIdList* idlist = m_vtkPolygon->GetPointIds();
	vtkPoints* points = m_vtkPolygon->GetPoints();
...
void RawDataPointmap::clearPointsSelection()
{
	m_selectedVerticesGrid->Reset();
...
void RawDataPointmap::clearNewPoints()
{
	m_newPoints->Reset();
...
void RawDataPointmap::resetSelectedInterp()
{
...
void RawDataPointmap::interpolatePoints(bool on)
{
	this->clearPointsSelection();
	...
	if (on){
		...
		InformationDialog::information(preProcessorWindow(), tr("Information"), tr("To interpolate points, select existing points by mouse-clicking, use Alt to define path between known points, and finish by double-clicking or pressing return key."), "rawdatapointmapinterpolatepoint");
		m_mouseEventMode = RawDataPointmap::meSMInterpPoint;
	} else {
		this->unwindSelectedInterp();
		this->resetSelectedInterp();
		//m_mouseEventMode = RawDataPointmap::meSMPointPrepare;
	}
	updateActionStatus();
...
void RawDataPointmap::enablePointSelectedActions(bool val)
{
...
QVector<vtkIdType> RawDataPointmap::selectedVertices()
{
...
void RawDataPointmap::updateBreakLinesOnDelete(QVector<vtkIdType>& deletedPoints)
{
...
void RawDataPointmap::updateBreakLinesOnInsert(QVector<vtkIdType>& deletedPoints)
{
	QList<vtkIdType> keyIds;
	QList<int> offsets;
...
void RawDataPointmap::doApplyOffset(double x, double y)
{
