#ifndef RAWDATAPOINTMAPNODEMAPPERT_H
#define RAWDATAPOINTMAPNODEMAPPERT_H
...
class RawDataPointmapNodeMapperSetting : public RawDataMapperSetting
{

public:
	RawDataPointmapNodeMapperSetting() : RawDataMapperSetting() {}
	~RawDataPointmapNodeMapperSetting() {}
	QList<DoubleMappingSetting> settings;
};

template <class V, class DA>
class RawDataPointmapNodeMapperT : public RawDataNodeMapperT<V, DA>
{

public:
	RawDataPointmapNodeMapperT(RawDataCreator* parent)
			...
			for (unsigned int i = 0; i < count; ++i) {
				...
				if (cellid >= 0) {
					...
					*(boolMap + i) = true;
					...
					RawDataNodeMapperT<V, DA>::m_grid->vtkGrid()->GetPointCells(i, cells);
					double bounds[6];
					for (vtkIdType j = 0; j < cells->GetNumberOfIds(); ++j) {
						vtkIdType cellid = cells->GetId(j);
						double tmpbounds[6];
						RawDataNodeMapperT<V, DA>::m_grid->vtkGrid()->GetCellBounds(cellid, tmpbounds);
						for (int k = 0; k < 3; ++k) {
...
#endif // RAWDATAPOINTMAPNODEMAPPERT_H
