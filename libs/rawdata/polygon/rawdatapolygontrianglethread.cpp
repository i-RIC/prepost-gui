...
RawDataPolygonTriangleThread::~RawDataPolygonTriangleThread()
{
	m_mutex.lock();
	m_abort = true;
	m_mutex.unlock();
...
void RawDataPolygonTriangleThread::cancel()
{
	QMutexLocker locker(&m_mutex);
	m_canceled = true;
...
void RawDataPolygonTriangleThread::run()
{
	forever {
		runTriangle();
...
void RawDataPolygonTriangleThread::runTriangle()
{
