...
RawDataPolygonAbstractPolygon::RawDataPolygonAbstractPolygon(RawDataPolygon* parent)
{
	m_parent = parent;
...
RawDataPolygonAbstractPolygon::~RawDataPolygonAbstractPolygon()
{
	vtkRenderer* r = m_parent->renderer();
...
bool RawDataPolygonAbstractPolygon::isVertexSelectable(const QVector2D& pos, double limitdist)
{
...
bool RawDataPolygonAbstractPolygon::isEdgeSelectable(const QVector2D& pos, double limitdist)
{
...
bool RawDataPolygonAbstractPolygon::isPolygonSelectable(const QVector2D& pos)
{
	QPolygonF pol = polygon();
	return pol.containsPoint(QPointF(pos.x(), pos.y()), Qt::OddEvenFill);
}

const QPolygonF RawDataPolygonAbstractPolygon::polygon(QPointF offset) const
{
...
void RawDataPolygonAbstractPolygon::setupContainers()
{
...
void RawDataPolygonAbstractPolygon::setupActors()
{
...
void RawDataPolygonAbstractPolygon::updateShapeData()
{
	vtkPoints* points = m_vtkPolygon->GetPoints();
	// update idlist.
	vtkIdList* idlist = m_vtkPolygon->GetPointIds();
...
void RawDataPolygonAbstractPolygon::updateScalarValues()
{
...
void RawDataPolygonAbstractPolygon::setZDepthRange(double /*min*/, double max)
{
...
void RawDataPolygonAbstractPolygon::setActive(bool active)
{
	...
	if (active) {
		...
		m_vertexActor->VisibilityOff();
		m_edgeActor->GetProperty()->SetLineWidth(RawDataPolygon::normalEdgeWidth);
	}
	m_parent->updateVisibilityWithoutRendering();
}
...
QPointF RawDataPolygonAbstractPolygon::innerPoint(QPointF offset) const
{
	double v[3];
...
bool RawDataPolygonAbstractPolygon::isClockwise() const
{
	double v[3];
	QVector2D basePoint, p1, p2, v1, v2, nom;
	QPointF nomP;
...
void RawDataPolygonAbstractPolygon::setLookupTable(vtkScalarsToColors* t)
{
	m_vertexMapper->SetLookupTable(t);
	m_edgeMapper->SetLookupTable(t);
}

void RawDataPolygonAbstractPolygon::setColor(const QColor& color)
{
	double rate = 0.8;
...
void RawDataPolygonAbstractPolygon::setMapping(RawDataPolygonColorSettingDialog::Mapping m)
{
	if (m == RawDataPolygonColorSettingDialog::Arbitrary) {
