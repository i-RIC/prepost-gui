...
Post2dWindowEdgeJScalarGroupTopDataItem::Post2dWindowEdgeJScalarGroupTopDataItem(Post2dWindowDataItem* p) :
	Post2dWindowScalarGroupTopDataItem{ tr("Scalar (edgeJ)"), QIcon(":/libs/guibase/images/iconFolder.png"), p }
{
...
{
	Q_ASSERT(node.toElement().nodeName() == "ScalarEdgeJ");
	if (node.toElement().nodeName() == "ScalarEdgeJ") {
...
void Post2dWindowEdgeJScalarGroupTopDataItem::mouseMoveEvent(QMouseEvent* event, VTKGraphicsView* v)
{
	...
	dynamic_cast<Post2dWindowZoneDataItem*>(parent())->updateEdgeJResultAttributeBrowser(QPoint(event->x(), event->y()), v);
}

void Post2dWindowEdgeJScalarGroupTopDataItem::mousePressEvent(QMouseEvent* event, VTKGraphicsView* v)
{
...
void Post2dWindowEdgeJScalarGroupTopDataItem::mouseReleaseEvent(QMouseEvent* event, VTKGraphicsView* v)
{
	...
	dynamic_cast<Post2dWindowZoneDataItem*>(parent())->fixEdgeJResultAttributeBrowser(QPoint(event->x(), event->y()), v);
}
...
class Post2dWindowEdgeJScalarGroupTopDataItem::CreateCommand : public QUndoCommand
{
public:
	CreateCommand(Post2dWindowEdgeJScalarGroupTopDataItem *item, QDialog* propDialog)
...
private:
	Post2dWindowEdgeJScalarGroupTopDataItem* m_topItem;
