	...
	Post2dWindowScalarGroupTopDataItem{ tr("Scalar (edgeI)"), QIcon(":/libs/guibase/images/iconFolder.png"), p }
{
	...
	for (std::string val : vtkDataSetAttributesTool::getArrayNamesWithOneComponent(cont->edgeidata()->GetPointData())) {
		...
		auto item = new Post2dWindowNodeScalarGroupDataItem(this, NotChecked, NotReorderable, NotDeletable, iRICLib::H5CgnsZone::SolutionPosition::IFace);
...
QDialog* Post2dWindowEdgeIScalarGroupTopDataItem::addDialog(QWidget* p)
{
	...
	dialog->setZoneData(zItem->dataContainer(), iRICLib::H5CgnsZone::SolutionPosition::IFace);
	if (! zItem->dataContainer()->IBCExists(iRICLib::H5CgnsZone::SolutionPosition::IFace)) {
		dialog->disableActive();
	}

	Post2dWindowContourSetting setting;
	setting.target = zItem->dataContainer()->data(iRICLib::H5CgnsZone::SolutionPosition::IFace)->GetPointData()->GetArrayName(0);
...
public:
	...
	void redo() {
		m_item = new Post2dWindowNodeScalarGroupDataItem(m_topItem, Checked, NotReorderable, Deletable, iRICLib::H5CgnsZone::SolutionPosition::IFace);
