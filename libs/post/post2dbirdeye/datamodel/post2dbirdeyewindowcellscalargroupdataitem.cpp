...
#include "private/post2dbirdeyewindowcellscalargroupdataitem_setsettingcommand.h"
...
std::string Post2dBirdEyeWindowCellScalarGroupDataItem::target() const
{
	return m_setting.target;
}

void Post2dBirdEyeWindowCellScalarGroupDataItem::setTarget(const std::string& target)
{
	m_setting.target = target.c_str();
	updateActorSettings();
}

void Post2dBirdEyeWindowCellScalarGroupDataItem::updateActorSettings()
{
	...
	Post2dBirdEyeWindowCellScalarGroupTopDataItem* topitem = dynamic_cast<Post2dBirdEyeWindowCellScalarGroupTopDataItem*>(parent());
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::setupIsolineSetting()
{
	Post2dBirdEyeWindowGridTypeDataItem* typedi = dynamic_cast<Post2dBirdEyeWindowGridTypeDataItem*>(parent()->parent()->parent());
	LookupTableContainer* stc = typedi->cellLookupTable(m_setting.target);
	...
	cellToPoint->SetInputData(m_valueClippedPolyData);
	cellToPoint->SetInputArrayToProcess(0, 0, 0, vtkDataObject::FIELD_ASSOCIATION_CELLS, iRIC::toStr(m_setting.target).c_str());

	m_warp->SetInputConnection(cellToPoint->GetOutputPort(0));
	m_warp->Update();

	m_isolineFilter->SetInputConnection(m_warp->GetOutputPort(0));
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::setupColorContourSetting()
{
	Post2dBirdEyeWindowGridTypeDataItem* typedi = dynamic_cast<Post2dBirdEyeWindowGridTypeDataItem*>(parent()->parent()->parent());
	LookupTableContainer* stc = typedi->cellLookupTable(m_setting.target);
	if (stc == nullptr) {return;}
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::setupColorFringeSetting()
{
	Post2dBirdEyeWindowGridTypeDataItem* typedi = dynamic_cast<Post2dBirdEyeWindowGridTypeDataItem*>(parent()->parent()->parent());
	LookupTableContainer* stc = typedi->cellLookupTable(target());
	if (stc == nullptr) {return;}
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::setupScalarBarSetting()
{
	Post2dBirdEyeWindowGridTypeDataItem* typedi = dynamic_cast<Post2dBirdEyeWindowGridTypeDataItem*>(parent()->parent()->parent());
	LookupTableContainer* stc = typedi->cellLookupTable(target());	
	if (stc == nullptr) {return;}
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::handlePropertyDialogAccepted(QDialog* propDialog)
{
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::createRangeClippedPolyData()
{
	PostZoneDataContainer* cont = dynamic_cast<Post2dBirdEyeWindowZoneDataItem*>(parent()->parent())->dataContainer();
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::createValueClippedPolyData()
{
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::updateVisibility(bool visible)
{
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::undoCommands(QDialog* propDialog, QUndoCommand* parent)
{
	Post2dWindowContourSettingDialog* dialog = dynamic_cast<Post2dWindowContourSettingDialog*>(propDialog);
...
void Post2dBirdEyeWindowCellScalarGroupDataItem::handleStandardItemChange()
{
