...
#include "post2dbirdeyewindowcellscalargroupdataitem_setsettingcommand.h"

#include <misc/stringtool.h>

#include <vtkScalarBarActor.h>

Post2dBirdEyeWindowCellScalarGroupDataItem::SetSettingCommand::SetSettingCommand(const Post2dWindowContourSetting& s, const LookupTableContainer& ltc, const QString& colorbarTitle, Post2dBirdEyeWindowCellScalarGroupDataItem* item, QUndoCommand* parent) :
	QUndoCommand {Post2dBirdEyeWindowCellScalarGroupDataItem::tr("Update Contour Setting"), parent},
	m_newSetting {s},
	...
	m_oldLookupTable {},
	m_item {item},
	m_topItem {dynamic_cast<Post2dBirdEyeWindowCellScalarGroupTopDataItem*>(item->parent())}
{
	...
	if (LookupTableContainer* lut = gtItem->cellLookupTable(item->target())) {
		m_oldLookupTable = *lut;
	}
}

void Post2dBirdEyeWindowCellScalarGroupDataItem::SetSettingCommand::redo()
{
	m_item->m_setting = m_newSetting;
	applySettings(m_newSetting.target, m_newLookupTable, m_newScalarBarTitle);
}

void Post2dBirdEyeWindowCellScalarGroupDataItem::SetSettingCommand::undo()
{
	m_item->m_setting = m_oldSetting;
	applySettings(m_oldSetting.target, m_oldLookupTable, m_oldScalarBarTitle);
}

void Post2dBirdEyeWindowCellScalarGroupDataItem::SetSettingCommand::applySettings(const QString& t, const LookupTableContainer& c, const QString& title)
{
	...
	if (LookupTableContainer* lut = gtItem->cellLookupTable(target)) {
		*lut = c;
		lut->update();
	}
