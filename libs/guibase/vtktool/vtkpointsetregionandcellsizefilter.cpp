...
void vtkPointSetRegionAndCellSizeFilter::calcExtractParameters(vtkStructuredGrid* data, const RectRegion& region, vtkAbstractPointLocator* pointLocator, int maxcell, bool* outOfRegion, int* imin, int* imax, int* jmin, int* jmax, int* rate)
{
	*outOfRegion = false;
	...
	// calc iMin1
	if (pointIsInsideRegion(region, data, dimensions, 0, centerJ)) {
		iMin1 = 0;
	} else {
		iMin1 = limitIIntersects(region, data, dimensions, centerI, 0, centerJ);
	}
	// calc iMax1
	if (pointIsInsideRegion(region, data, dimensions, *(dimensions) - 1, centerJ)) {
		iMax1 = *(dimensions) - 1;
	} else {
		iMax1 = limitIIntersects(region, data, dimensions, centerI, *(dimensions) - 1, centerJ);
	}
	// calc jMin1
	if (pointIsInsideRegion(region, data, dimensions, centerI, 0)) {
		jMin1 = 0;
	} else {
		jMin1 = limitJIntersects(region, data, dimensions, centerJ, 0, centerI);
	}
	// calc jMax1
	if (pointIsInsideRegion(region, data, dimensions, centerI, *(dimensions + 1) - 1)) {
		jMax1 = *(dimensions + 1) - 1;
	} else {
		jMax1 = limitJIntersects(region, data, dimensions, centerJ, *(dimensions + 1) - 1, centerI);
	}
	...
	if (iMin1 == 0) {
		...
		if (iLineIntersectsRegion(region, data, dimensions, 0)) {
			iMin2 = 0;
		} else {
			iMin2 = limitIIntersects(region, data, dimensions, centerI, 0);
		}
	}
	// calc iMax2
	if (iMax1 == *(dimensions) - 1) {
		...
		if (iLineIntersectsRegion(region, data, dimensions, *(dimensions) - 1)) {
			iMax2 = *(dimensions) - 1;
		} else {
			iMax2 = limitIIntersects(region, data, dimensions, centerI, *(dimensions) - 1);
		}
	}
	// calc jMin2
	if (jMin1 == 0) {
		...
		if (jLineIntersectsRegion(region, data, dimensions, 0)) {
			jMin2 = 0;
		} else {
			jMin2 = limitJIntersects(region, data, dimensions, centerJ, 0);
		}
	}
	// calc jMax2
	if (jMax1 == *(dimensions + 1) - 1) {
		...
		jMax2 = limitJIntersects(region, data, dimensions, centerJ, *(dimensions + 1) - 1);
	}
